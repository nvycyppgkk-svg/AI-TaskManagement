...
| C-8 | カードをドラッグ&ドロップでリスト間の移動・同一リスト内での並び替えができる（手動並び替えモード時） |
| C-10 | リスト単位で優先度順にカードを並び替えられる |
| C-11 | リスト単位で期限順にカードを並び替えられる（期限なしは末尾） |
| C-12 | 並び替えモード（優先度順・期限順）はリストごとに独立して設定できる |
| C-13 | 並び替えモード中はドラッグ&ドロップが無効になる |
