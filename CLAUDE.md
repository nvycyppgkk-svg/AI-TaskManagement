...
- base branchは必ず `main`
- **PR本文に必ず `closes #{Issue番号}` を含める**（mainにマージされると対応Issueが自動クローズされる）
...
このプロジェクトは以下の固定ポートを使用する。**別ポートでの一時起動は禁止。**

| サービス | ポート | 起動コマンド |
|---|---|---|
| バックエンド（Spring Boot） | `8080` | `cd backend && ./gradlew bootRun` |
| フロントエンド（Vite） | `5173` | `cd frontend && npm run dev` |
| DB（PostgreSQL） | `5432` | Docker Compose |

### ポート競合時の対処（MUST）

サーバーを起動する前に、対象ポートを使用しているプロセスが存在する場合は、**必ずそのプロセスを停止してから**指定ポートで起動すること。別ポートへの変更は禁止。
...
# 競合プロセスを停止（PIDを確認してkill）
kill <PID>
# または
pkill -f "gradlew\|bootRun"   # バックエンド停止
pkill -f "vite"                # フロントエンド停止
```

**手順：**
1. 起動前に `lsof -i :<ポート>` で競合確認
2. 競合があれば該当プロセスを停止
3. 指定ポートで起動
